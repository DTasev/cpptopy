...
// Example 3
// Load PNG file from disk using a State, normally needed for more advanced
// usage.
void decodeWithState(const char *filename) {
  ...
  unsigned error = lodepng::load_file(
      png, filename); // load the image file with given filename
  if (!error)
  ...
  // the pixels are now in the vector "image", 4 bytes per pixel, ordered
  // RGBARGBA..., use it as texture, draw it, ...  State state contains extra
  // information about the PNG such as text chunks, ...
}

int main(int argc, char *argv[]) {
  const char *filename = argc > 1 ? argv[1] : "../img.png";
